...
async def perform_ocr(request: OCRRequest) -> OCRResponse:
    ...
    if request.ocr not in ["easyocr", "llm_ocr"]:
        raise HTTPException(
            status_code=400,
            detail="OCR method must be 'easyocr' or 'llm_ocr'"
        )
    ...
    try:
        ...
        if request.ocr == "easyocr":
            service = easyocr_service
        else:
            service = llm_ocr_service
        
        # Extract raw text
        raw_ocr = await service.extract_text(str(file_path))
        
        # Parse fields
        parsed_result = await service.parse_fields(raw_ocr, request.fields)
